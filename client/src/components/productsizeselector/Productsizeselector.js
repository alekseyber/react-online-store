...
const ProductSizeSelector = ({ level2, alias, error, set_error, sizesgroup_id }) => {
    ...
    const SizeItem = ({ item }) => {
        ...
        const variant = (selectSize === item) ? 'contained' : 'outlined';
        ...
        return <Button
            ...
            size="small"
        >{sizesData[item].title}</Button>
    }


    return (
        <>
            <Typography variant="subtitle1" component="div" color={labelColorText}>{labelText}</Typography>
            {error && <Typography variant="caption" color="error">*Размер можно скорректировать при подтверждении заказа менеджером</Typography>}
            ...
            {sizesgroup_id && <Button onClick={handleSizeChart} size="small" className="mt-1">Размерная сетка</Button>}
        </>
    )
