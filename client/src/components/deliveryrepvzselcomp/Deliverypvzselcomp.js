...
import { pvzSelectVar } from "../../graphql/localVars";
import { DELIVERY_PVZ_SEL_COMP_QUERY } from "../../graphql/gqlQuery";
...
const ListItemPvz = ({ item, pvzSelect, index, handleSetPvz }) => {
...
const MapItemPvz = ({ item, handleSetPvz, index, pvzSelect }) => {
  let selected = false;
  if (pvzSelect) {
    selected = item.code === pvzSelect.Code;
  }
...
const MapPvz = ({ pvz, pvzSelect, handleSetPvz, yaMapKey }) => {
  const classes = useStyles();
...
const greatItem = (item, index, cityid) => {
  const rezult = {
    index,
    cityid,
    Code: item.code,
...
const handleSetPvzFunc = (cityid, item, index) => {
  const selectedPvzValue = greatItem(item, index, cityid);
  pvzSelectVar(selectedPvzValue);  
};

const DeliveryPvzSelComp = ({ dataInput }) => {
  const classes = useStyles();
  const theme = useTheme();
  const { data } = useQueryApp(DELIVERY_PVZ_SEL_COMP_QUERY);
  const pvzSelect = data.pvzSelect;
