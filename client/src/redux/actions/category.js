...
export const categoryFetch = (alias) => async (dispatch, getState) => { //, getState
    try {

        const { category, start } = getState();

        // const regex1 = /([^A-Za-z0-9-]+)/;
        // alias = alias.replace(regex1, '');
        ...
        newcategoryData.productsData.countModif = categoryData.productsData.countModif;
        newcategoryData.productsData.countProduct = categoryData.productsData.countProduct;
...
export const categoryUpdateSort = (alias, sortValue) => (dispatch, getState) => {
