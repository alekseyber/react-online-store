...
export const cartAddCupon = (cupontext) => async (dispatch) => {
  //, getState
  ...
  try {
    ...
    if (cupontext.length > 5 && cupontext.length < 12) {
      const cuponData = await requestFormLoader("/api/order/getcupon", "get", {
        cupontext,
      });
...
export const cartChangeItemCount = (index, qty) => (dispatch, getState) => {
  //, getState
  const { cart } = getState();
...
export const cartAddPageAction = (id) => async (dispatch) => {
  //, getState

  const { requestRedirect } = httpActions(dispatch);

  try {
    const { alias, level1, level2, price } = await requestRedirect(
