...
export const updateProducts = (products, byalias = false, returnRez = false) => async (dispatch, getState) => { //, getState
    try {
        ...
        if (ids.length > 0) {
            ...
            const productsData = await requestNoLoader('/api/products/getproductsbyids', 'get', params);
...
export const mainProductFetch = (alias) => async (dispatch, getState) => {
    ...
    try {
        ...
        if (mainLoad) {
            const productContentData = await requestRedirect(`/api/products/getproductcontent/${alias}`);
