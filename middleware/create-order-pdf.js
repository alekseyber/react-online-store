...
async function getParams() {
    try {
        const doc = await Params.findOne({ select: true }, { _id: 0, logoimg: 1, shop_name: 1, shop_name_rus: 1, phone: 1, currSymbol: 1, baseUrl: 1, orderPrintText: 1 });
        if (doc) {
            return doc;
        }
...
function getStartdocDefinition(params, order) {
    ...
    return docDefinition = {
        info: {
            title: `Заказ № ${order.orderNum} от ${moment(order.createdAt).format("DD.MM.YYYY")}`,
...
function getOrderContet(params, order) {
    ...
    const productSumm = [
        {
            colSpan: 4,
            text: `Итого товаров на сумму, ${params.currSymbol}`, bold: true
        },
        '',
        '',
        '',
        {
            colSpan: 2,
            text: order.summa, alignment: 'right'
        },
        ''
    ];
    ...
    const divelSumm = [
        {
            colSpan: 4,
            text: `Доставка, ${params.currSymbol}`, bold: true
        },
        '',
        '',
        '',
        {
            colSpan: 2,
            text: order.deliveryPrice, alignment: 'right'
        },
        ''
    ];
    cart.push(divelSumm);
    ...
    const summ = [
        {
            colSpan: 4,
            text: `Итого, ${params.currSymbol}`, bold: true
        },
        '',
        '',
        '',
        {
            colSpan: 2,
            text: (order.summa + order.deliveryPrice), bold: true, alignment: 'right'
        },
        ''
    ];
    cart.push(summ);
    
    const imgPrefix = /\.png$/.test(logoImg) ? 'data:image/png;base64,' : 'data:image/jpg;base64,';
    ...
    dd.content = [
        {
            ...
            columns: [
                {
                    image: imgPrefix + logoImg,
                    width: 90
                },
                {
                    qr: String(order._id), fit: '50'
                },
                {
                    stack: [
                        { text: order.cityName, style: 'header' },
                        order.oblName
                    ],
                    width: 170,
                }
                ...
                body: [
                    [
                        ...
                        },
                        {
                            border: [true, true, true, false],
                            fillColor: '#eeeeee',
                            text: 'ИНФОРМАЦИЯ О ЗАКАЗЕ', bold: true,
                        }
                    ],
                    [
                        {
                ...
                },
                {
                    stack: [
                        ...
                        },
                        {
                            margin: [0, 0, 0, 15],
                            style: 'bottomleft',
                            text: 'ПОДТВЕРЖДЕНИЕ ДОСТАВКИ', bold: true,
                        },
                        {
                            style: 'bottomleft',
                        ...
                        },
                        {
                            margin: [0, 15, 0, 0],
                            style: 'bottomleft',
                            text: 'ПОЛУЧАТЕЛЬ__________________________________________', bold: true,
                        },
                    ],
                }
            ]
