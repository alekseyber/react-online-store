...
class Field {
    constructor(schematype, modelName) {
        ...
        switch (schematype.instance) {
            case 'String':
                this.value = ('defaultValue' in schematype) ? schematype.defaultValue : '';
            ...
            case 'Number':
                ...
                break;
            case 'Date':
                this.value = ('defaultValue' in schematype) ? new Date().toISOString() : null;

                if (schematype.path === 'createdAt' || schematype.path === 'update_at') {
                    this.type = 'timestamps';
                    this.labeltext = (schematype.path === 'createdAt') ? 'Создан' : 'Изменен';
                } else {
                    this.type = 'datepicker';
                }
                break;
            case 'Boolean':
                this.value = ('defaultValue' in schematype) ? schematype.defaultValue : false
                break;
            case 'ObjectID':
                if (this.path !== '_id') {
            ...
            case 'Array':
                ...
                if (this.schema) {
                    ...
                    this.value = ('default' in schematype.options) ? schematype.options.default : [];
