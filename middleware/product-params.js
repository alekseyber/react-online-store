...
const getBrands = async () => {
  try {
    const rezult = {};
    brands = await Brand.find({}, { title: 1, img: 1 }).sort({
      sortvalue: 1,
    });
    brands.forEach((item) => {
      rezult[item._id] = {
...
const getProductPatternData = async (
  product,
  level1,
  currSymbol,
  level1Obj = true
  ...
  try {
    ...
    patternData.product_gender_pril = patternData.product_gender.replace(
      /(е+$)/iu,
      "х"
    );
    ...
    patternData.product_name = patternData.product_name_full
      .replace(/([а-я]+)/iu, "")
      .trim();
    patternData.product_name_full = product.title;
    ...
    if (product_price_discont_value > 0) {
      patternData.product_price_discont_proc =
        product.price / (product.old_price / 100);
      patternData.product_price_discont_proc = Math.round(
        100 - patternData.product_price_discont_proc
      );
      patternData.product_price_discont_proc += "%";
      ...
      patternData.product_price_discont_procoi =
        " со скидкой " + patternData.product_price_discont_proc;
      patternData.product_price_discont_proc =
        " скидка " + patternData.product_price_discont_proc;
      patternData.product_price_discont_value =
        "экономия " + product_price_discont_value + " " + currSymbol;
    ...
    const colorsKeys = level1Obj
      ? Object.keys(level1)
      : level1.map((lev) => lev.level1_alias);
