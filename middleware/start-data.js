    ...
    for (let i = 0; i < colors.length; i++) {
      if (colors[i].children.length > 0) {
        ...
        for (let s = 0; s < colors[i].children.length; s++) {
          rezult.colors[colors[i].children[s].aliasitem] =
            colors[i].children[s];
          rezult.colorsGroup[colors[i].alias].children.push(
            colors[i].children[s].aliasitem
          );
        }
      }
    }
...
module.exports.getSort = async () => {
  ...
  try {
    rezult.sortList = await Sort.find(
      {},
      { _id: 1, text: 1, field: 1, order: 1, sort_default: 1 }
    ).sort({ sortvalue: 1 });
