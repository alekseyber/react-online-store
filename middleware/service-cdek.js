...
const addOrderStatuses = {
    ...
    '4': {
        name: 'Возврат, не должен выполняться: вес отличается от заявленного более, чем на X г.',
        provided: false
    },
    '5': {
        name: 'Возврат, не должен выполняться: фактически нет отправления (на бумаге есть)',
        provided: false
    },
    '6': {
        name: 'Возврат, не должен выполняться: дубль номера заказа в одном акте приема-передачи',
        provided: false
    },
    '7': {
        name: 'Возврат, не должен выполняться: не доставляем в данный город(регион)',
        provided: false
    },
    '8': {
...
async function getDeliverySettings(projectionArr = ['api_settings']) {
    try {
        ...
        const deliverysettings = await Deliverysettings.findOne({ vendor: 'cdek' }, projection);
...
async function getProducts(cart, package_base) {
    ...
    try {
        ...
        if (ids.length) {
            const products = await Product.find({ _id: { $in: ids } }, { sku: 1, packages_id: 1 })
                .populate({ path: 'packages_id', select: { _id: 0, weight: 1, length: 1, width: 1, height: 1, } });
            ...
            if (products.length) {
                ...
                products.forEach(item => {
                    ...
                    const packages_item = item.packages_id ? item.packages_id : package_base;
...
async function clientAuthorization(api_settings = false, dev = false) {
    try {
        const candidate = await Cacheauth.findOne({ cacheKey: 'cdek' }, { _id: 0, cacheValue: 1 });

        if (candidate) {
            axios.defaults.headers.common = { 'Authorization': `Bearer ${candidate.cacheValue}` };
            return axios
        }
        const oauthSettings = {
            url: '',
        ...
        if (dev) {
            ...
            if (api_settings === false) {
                ...
                oauthSettings.client_secret = doc_api_settings.api_settings.client_secret;
            } else {
...
function orderValidate(order) {
    try {
        ...
        if (order.deliverySelect === 0 && order.street.length === 0 && order.house.length === 0) {
...
class OrderCdek {
    constructor(order, deliverysettings, products) {
        ...
        this.tariff_code = (order.deliverySelect === 0) ?
        ...
        if (deliverysettings.sender_status) {
            ...
            Object.keys(deliverysettings.sender).forEach(key_sender => {
                if (deliverysettings.sender[key_sender] && key_sender !== '$init' && key_sender !== 'phones') {
        ...
        this.from_location = {};
        this.from_location.code = deliverysettings.from_location.code ? deliverysettings.from_location.code : '';
        this.from_location.fias_guid = deliverysettings.from_location.fias_guid ? deliverysettings.from_location.fias_guid : '';
        this.from_location.postal_code = deliverysettings.from_location.postal_code ? deliverysettings.from_location.postal_code : '';
        this.from_location.longitude = deliverysettings.from_location.longitude ? deliverysettings.from_location.longitude : '';
        this.from_location.latitude = deliverysettings.from_location.latitude ? deliverysettings.from_location.latitude : '';
        this.from_location.country_code = deliverysettings.from_location.country_code ? deliverysettings.from_location.country_code : '';
        this.from_location.region = deliverysettings.from_location.region ? deliverysettings.from_location.region : '';
        this.from_location.sub_region = deliverysettings.from_location.sub_region ? deliverysettings.from_location.sub_region : '';
        this.from_location.city = deliverysettings.from_location.city ? deliverysettings.from_location.city : '';
        this.from_location.kladr_code = deliverysettings.from_location.kladr_code ? deliverysettings.from_location.kladr_code : '';
        this.from_location.address = deliverysettings.from_location.address ? deliverysettings.from_location.address : '';
        ...
        order.cart.forEach(product => {
            productPackagesArr.push(products.productsObj[product.product_id].packages)
...
module.exports.orderRegistration = async (order_id, dev = false) => {
    ...
    try {
        ...
        if (orderValidate(order)) {
            ...
            const products = await getProducts(order.cart, deliverysettings.package_base);
            const orderCdek = new OrderCdek(order, deliverysettings, products);

            let $axios = await clientAuthorization(deliverysettings.api_settings, dev);
            ...
            const message = request.warnings.length ? request.warnings[0].message : 'Ошибка регистрации заказа'
...
module.exports.deletingOrder = async (uuid, dev = false) => {
    try {
        ...
        if (request.errors.length) {
            const message = request.warnings.length ? request.warnings[0].message : 'Ошибка в ответе СДЕК'
            throw new Error(message);
        }
...
module.exports.creatingOrderReceipt = async (order_uuid_arr, copy_count = 2, action = 1, byCdekNumber = false, dev = false) => {
    try {
...
module.exports.receivingOrderReceipt = async (uuid, action = 1, dev = false) => {
    try {
        ...
        if (endStatuse.code === 'READY' && data.entity.url) {
            ...
            throw new Error('Документ еще не сформирован, текущий статус: ' + endStatuse.name);
        }
...
module.exports.creatingOrderReceiptByInvoice = async (order_id, dev = false) => {

    try {

        const order_doc = await getOrderById(order_id, ['deliveryUuid', 'deliveryStatus', 'deliveryInvoice', 'deliveryInvoiceHand']);
        ...
        if (order_doc.deliveryInvoice) {
            ...
            if (rezult.entity.orders.length) {
                ...
                const message = request.warnings.length ? request.warnings[0].message : 'Заказ не найден по номеру'
                throw new Error(message);
            }
        } else {
