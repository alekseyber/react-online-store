...
async function getIndexProduct($regex) {
    try {
        ...
        if (aggregate.length === 1) {
            ...
            const sort = await Sort.findOne({ sort_default: true }, { order: 1, field: 1 });
            let sort_obj = {};
            if (sort) {
        ...
        console.error(e);
        throw "Ошибка БД getIndexProduct";
    }
...
module.exports.searchList = async (req, res) => {
    ...
    try {
        ...
        const queryParams = getQuery(req.query.q);
        let rezult = {
            products: [],
            searchAll: false
        }
        ...
        if (queryParams.valid) {
            ...
            docs = await Indexproduct.find(query, { _id: 0, product_id: 1, title: 1, link: 1 }).sort({ product_id: 1, sortvalue: 1 }).limit(1300);
            let product_ids = [];
            for (let doc of docs) {
