    ...
    try {
        ...
        if (Model && _id !== undefined && formData !== undefined) {
            ...
            Object.keys(formData).forEach(el => {
                if (el !== '_id' && el !== '__v' && el !== 'createdAt' && el !== 'update_at') {
                    proection[el] = true;
                }
            });
            ...
            if (doc) {
                ...
                if (modelName === 'order') {
                    if (doc.orderStatus_id && formData.orderStatus_id) {
                        if (String(doc.orderStatus_id) !== String(formData.orderStatus_id)) {
...
module.exports.deleteModelById = async (req, res) => {
    try {
        ...
        if (Model && _id !== undefined) {
            ...
            const deleteResponse = await Model.deleteOne({ _id: Types.ObjectId(_id) });
    ...
    try {
        ...
        if (Models) {
            ...
            rezult.models.forEach(modelName => {
                rezult.modelFields[modelName] = Object.keys(Models[modelName].schema.paths)
...
module.exports.getSatisStatistic = async (req, res) => {
    try {
        ...
        const statisticArrGrupp = ['product', 'order', 'news', 'comment', 'acquirer', 'category', 'ordergraf'];
        const gruppsRezult = {};
