    ...
    try {
        ...
        if (cacheData === null) {
            ...
            const aggregate = await Product.aggregate([
                ...
                { $match: { amount: { $ne: 0 } } },
                { $group: { _id: "products", colors: { $addToSet: "$colors" }, sizes: { $addToSet: "$sizes" }, ids: { $addToSet: "$id" } } }

            ])

            const filters_query = Product.distinct("filter", { _id: { $in: aggregate[0].ids } }, function (err, docs) {
                if (!err) {
                    return docs;
                } else {
                    return null;
            ...
            const getSortedIndexList = (list) => Object.keys(list).sort((a, b) => list[a].sortvalueitem - list[b].sortvalueitem);
            ...
            const filterData = await Filter.find({ status: true, filter: true }, { _id: 0, color: 1, sizes: 1, title: 1, alias: 1, attrs: 1 }).sort({ sortvalue: 1 });
            let filterRezult = []
            let filterIndex = [{}, {}];
                ...
                filterData.forEach((grupp) => {
                    ...
                    if (grupp.color) {
                        ...
                        keysSorted.forEach((index) => {
                            if (tempAttrs[index].status_attr && rezult.filter[tempAttrs[index].alias_attrs] !== undefined) {
            ...
            cacheData = {};
            cacheData['obj'] = { filterRezult, filterIndex, colorsGrupp };
            const сache = new Cache({ cacheKey, cacheData, cacheAction });
            await сache.save();
            return res.status(200).json(cacheData.obj)

        } else {
            //  console.log(cacheData.cacheData.get('obj'));
            return res.status(200).json(cacheData.cacheData.get('obj'))
        }
