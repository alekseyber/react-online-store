...
const { sendAdminEmailFromOrder, sendAdminEmailReturnProduct, sendAdminEmailReturnCall } = require('../emails/sendmail');
...
// async function productValidate(product_id_text, level1_alias, level2_alias) {
//     try {
//         const product_id = Types.ObjectId(product_id_text);
...
async function detectAcquirer(phone, status_block = false) {

    let rezult = false;
    try {

        const acquirer = await Acquirer.findOne({ phone, status_block }, { phone: 1 });
        if (acquirer) {
            rezult = acquirer._id;
        }
...
async function detectReturnProduct(acquirer_id) {

    let rezult = true;
    try {
        const doc = await Returnproduct.findOne({ acquirer_id }, { _id: 0, status: 1 });
        if (doc) {
            rezult = false;
        }
...
async function productValidate(alias, level1_alias, level2_alias) {
    try {
        //  const product_id = Types.ObjectId(product_id_text);
        const aggregate = await Product.aggregate([
            {
...
async function getCartValidate(cart, discontcupon) {
    try {

        let summa = 0;
        const cartData = [];

        const docParams = await Params.findOne({ select: true }, { _id: 0, productImgProperty: 1 });
        const imgPath = docParams.productImgProperty.filter(el => el.status === 'thumb')[0].path;
        ...
        for (const itemCart of cart) {
            ...
            //    item.title = itemCart.title; //
            //   item.img = itemCart.img_main; //
            //  item.link = itemCart.link; //
            ...
            //  item.product_id = itemCart.id_pr //
            ...
            item.valid = false;
            const validItem = await productValidate(itemCart.alias, item.level1_alias, item.level2_alias);
            if (validItem) {
                ...
                if ((validItem.product_model < 3) && colors[item.level1_alias]) {
                    ...
                    // if (itemCart.color_title !== "false") {
                    //     item.title = item.title + " " + itemCart.color_title;
                    // } else {
                    //     item.title = item.title + " " + item.level1_alias;
                    // }
                }

                if ((validItem.product_model === 1 || validItem.product_model === 4) && sizesData[item.level2_alias]) {
                    //  item.link = addGet(item.link, "sizes", item.level2_alias);

                    item.title = item.title + " р." + sizesData[item.level2_alias].title;

                    // if (itemCart.sizes_title !== "false") {
                    //     item.title = item.title + " р." + itemCart.sizes_title;
                    // } else {
                    //     item.title = item.title + " р." + item.level2_alias;
                    // }
                }
            }
            cartData.push(item);
        ...
        console.error(e.message);
        throw "Ошибка обработки корзины"
    }
}

async function cuponValidate(cupon_id) {
    let rezult = {
        status: false,
        discont: 1,
        cupon_id: ""
    ...
    try {
        ...
        const doc = await Cupon.findOne({ _id: cupon_id, status: true, expiryDate: { $gte: startDate } });
    ...
    try {
        ...
        if (validInput) {
            ...
            if (inputData.deliveryPrice !== 'false' && inputData.deliveryPrice) {
                order.deliveryPriceCourier = Number(inputData.deliveryPrice.courier.priceByCurrency);
                order.deliveryPricePvz = Number(inputData.deliveryPrice.pvz.priceByCurrency);
            ...
            if ((inputData.pvzSelect && inputData.pvzSelectStatus) && (inputData.pvzSelect !== 'false' && inputData.pvzSelectStatus !== 'false')) {
            ...
            res.status(201).json(rezultOrder);
        } else {
            res.status(400).send('Ошибка передачи данных');
        }

    } catch (e) {
        console.error(e.message);
        res.status(500).send('Получена ошибка на сервере. Повторите попытку позже.');
    }
...
module.exports.getCupon = async (req, res) => {
    try {
        const cupontext = String(req.query.cupontext);
        ...
        if (cupontext.length > 5 && cupontext.length < 12) {
            ...
            startDate.setMinutes(0);
            startDate.setSeconds(0);
            const doc = await Cupon.findOne({ cupontext: cupontext, status: true, expiryDate: { $gte: startDate } });
        ...
        if (rezultStatus) {
            ...
            res.status(404).send('Купон не найден');
        }
...
module.exports.fetchById = async (req, res) => {
    try {
        ...
        if (id) {
            Order.findById(id, {}, async function (err, doc) {
                if (doc) {
...
module.exports.returnProductForm = async (req, res) => {
    ...
    try {
        const inputData = req.body;
        const acquirer_ip = detectIP(req);
        const recaptchaStatus = await verifyingGRecaptcha(inputData.recaptchaToken, acquirer_ip);
        if (recaptchaStatus) {
            const phone = phoneFormat(inputData.phone);
...
module.exports.returnCallForm = async (req, res) => {
    ...
    try {
        const inputData = req.body;
        const user_ip = detectIP(req);
        const recaptchaStatus = await verifyingGRecaptcha(inputData.recaptchaToken, user_ip);
        if (recaptchaStatus) {
            const phone = phoneFormat(inputData.phone);
            const name = String(inputData.name.trim());
            const comment = String(inputData.comment.trim());
            if (phone) {
                let rezultObj = { phone, name, comment, user_ip };
